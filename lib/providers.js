...
const LISTERS = {
  ...
  async gemini() {
    // The native endpoint tells us which models really support chat generation.
    const key = env("GEMINI_API_KEY");
    const data = await getJson(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(key)}&pageSize=200`);
    return (data.models || [])
      .filter((m) => (m.supportedGenerationMethods || []).includes("generateContent"))
      .map((m) => ({ id: m.name.replace(/^models\//, ""), label: m.displayName || m.name, desc: `${m.description || ""}` }))
      .filter(({ id, desc }) =>
        /^(gemini|gemma)/.test(id) &&
        !/(embed|tts|image|live|audio|aqa|native|robotics|computer|thinking-exp|exp-)/.test(id) &&
        !/deprecat|discontinu|no longer/i.test(desc))
      .filter(({ id }) => !/pro/.test(id)) // Pro models are not on the free tier
      .sort((a, b) => b.id.localeCompare(a.id, undefined, { numeric: true }))
      .map(({ id, label }) => ({ model: id, label: label.replace(/\s*\(.*\)$/, ""), vision: true }));
  },
...
/* ---------------- model health ---------------- */
// Which models actually answered last time they were tried. Kept in memory and
// saved by the server into the admin account, so it survives a restart.
let health = {};
export const getHealth = () => health;
export const setHealth = (next) => { health = next || {}; };
export function noteHealth(modelId, ok, error) {
  health[modelId] = { ok, error: ok ? null : String(error || "").slice(0, 200), at: new Date().toISOString() };
  return health[modelId];
}

/** Sends one tiny message to check whether a model really works. */
export async function testModel(modelId) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 25000);
  try {
    for await (const chunk of streamChat({ modelId, messages: [{ role: "user", content: "Reply with the word OK." }], system: "Reply with the word OK.", signal: controller.signal })) {
      if (chunk.trim()) return noteHealth(modelId, true);
    }
    return noteHealth(modelId, false, "The model replied with nothing.");
  } catch (err) {
    if (err.name === "AbortError") return noteHealth(modelId, false, "Timed out after 25 seconds.");
    return noteHealth(modelId, false, err.message);
  } finally {
    clearTimeout(timer);
  }
}

/** Models to try instead of one that just failed: same provider first, then other free ones. */
export async function fallbackModels(modelId, { needsVision = false, limit = 2 } = {}) {
  const { models } = await listModels();
  const provider = modelId.slice(0, modelId.indexOf(":"));
  const usable = models.filter((m) => m.id !== modelId && health[m.id]?.ok !== false && (!needsVision || m.vision));
  const score = (m) => (m.provider === provider ? 0 : 1) + (m.tier === "free" ? 0 : 2) + (health[m.id]?.ok ? -0.5 : 0);
  return usable.sort((a, b) => score(a) - score(b)).slice(0, limit);
}
